...
atualizarS<-function(dados,p,mu,covariaveis,beta,sigma2,lambda,nu,N,numcomp,numcov){
  ...
  for(g in 1:numcomp){
    ...
    matrizaux[g,]<-p[g]*densSSL(dados,escala,sigma2[g],lambda[g],nu[g],N)
  }
...
# Full conditional for latent varible in asymmetry
...
atualizarU<-function(nu,dados,x,beta,mu,psi,tau,z,N){
  alpha<-c(rep(nu+1,N))
  beta<-(dados-mu-x%*%beta-psi*z)^2/(2*tau^2)+0.5*z^2
  aux<-runif(N)
